...
app.get('/api/v1/content/download/:jobId', async (req, res) => {
  ...
  try {
    ...
    // SECURITY (CodeQL js/path-injection — alerts 36,37,38): `jobId` is a
    // user-controlled route parameter. Validate it as a strict flat filename
    // (no separators, no `..`, no null bytes, allow-listed characters) and
    // resolve all paths within a fixed base directory so traversal attempts
    ...
    try {
      ...
      logger.warn('Rejected download request - invalid jobId', {
        jobId,
    ...
    let zipFilePath;
    try {
      jobDirectory = resolveWithinBase(generatedRoot, safeJobId);
      zipFilePath = resolveWithinBase(jobDirectory, `${safeJobId}_press_releases.zip`);
    } catch (containmentError) {
      logger.warn('Rejected download request - path escapes storage root', {
        jobId,
        reason: containmentError.message
      });
      return res.status(400).json({
        error: 'Invalid job ID',
        message: 'jobId resolves outside the allowed storage directory',
        timestamp: new Date().toISOString()
      });
    }

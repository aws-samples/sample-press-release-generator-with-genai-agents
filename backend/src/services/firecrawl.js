...
class FirecrawlService {
  ...
  _buildAuthoritativeSearchUrl(source, searchTerms) {
    ...
    if (source.includes('nar.competitor2')) {
      return `${source}/research-and-statistics?search=${encodedQuery}`;
    } else if (source.includes('example.com')) {
      return `${source}?s=${encodedQuery}`;
    } else if (source.includes('competitor1.com')) {
      return `${source}?q=${encodedQuery}`;
    } else if (source.includes('competitor2.com')) {
      return `${source}?q=${encodedQuery}`;
    } else if (source.includes('housingwire.com')) {
      return `${source}?s=${encodedQuery}`;
    } else if (source.includes('fred.stlouisfed.org')) {

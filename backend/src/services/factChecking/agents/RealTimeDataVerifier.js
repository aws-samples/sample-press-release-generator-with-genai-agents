...
class RealTimeDataVerifier {
  ...
  _calculateUrlCredibility(url, source) {
    ...
    if (urlLower.includes('nar.competitor2') ||
        urlLower.includes('fred.stlouisfed.org') ||
        urlLower.includes('census.gov')) {
      return 0.95;
    }
    
    // Tier 2: Industry sources (0.8-0.85)
    if (urlLower.includes('example.com') ||
        urlLower.includes('competitor1.com') ||
        urlLower.includes('competitor2.com') ||
        urlLower.includes('housingwire.com')) {
      return 0.85;
    }
    
    // Tier 3: Financial news (0.75-0.8)
    if (urlLower.includes('reuters.com') ||
    ...
    if (urlLower.includes('inman.com') ||
        urlLower.includes('bizjournals.com')) {
      return 0.7;
    }
  ...
  _buildSearchUrl(query, sourceInfo, dataType) {
    ...
    if (sourceInfo.type === 'search_aggregator') {
      if (sourceInfo.url.includes('google.com')) {
        return `${sourceInfo.url}/search?q=${encodedQuery}`;
      } else if (sourceInfo.url.includes('brave.com')) {
        return `${sourceInfo.url}/search?q=${encodedQuery}`;
      } else if (sourceInfo.url.includes('duckduckgo.com')) {
